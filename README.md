...
Streaming response from a Lambda function is possible through **function URLs**.

* Create a user in the user pool (generated by the stack) and add it to the `fn-url-access` group.
* Navigate to the `frontend` folder: `cd frontend`.
* Create a file called `.env` in the folder. Copy the `VITE_USER_POOL_ID`, `VITE_USER_POOL_CLIENT_ID`, `VITE_IDENTITY_POOL_ID` and `VITE_LAMBDA_FUNCTION_URL` environment variable values from the CloudFormation stack **Outputs**.
* Run `npm install`
* Run `npm run dev`
* Open `http://localhost:5173/login` page in a browser tab.
* Enter your user's username and password.

Enter a prompt and select the use case. The response will gradually appear on the screen as the chunks arrive through the Lambda function URL.
